...
#####mit:
* dem vorangestellten Punkt: __&middot;__ | wird der Bezeichner als Klassen-Selektor interpretiert <br>Beispiel: `<p class="testKlasse">` wird in CSS über `.testKlasse { … }`selektiert

* dem vorangestellten Hash: __#__ | wird der Bezeichner als ID-Selektor interpretiert <br>Beispiel: `<p id="testID">` wird in CSS über `#testID { … }`selektiert
...
* Mehrfach-Selektion ist möglich über die Verwendung von Kommata. <br>Beispiel: Die Elemente `<body> … </body>` `<p id="testID"> … </p>` `<h1> … </h1>` werden in CSS über `body, #testID, h1 { … }` gemeinsam selektiert
* Die Kombination von generischen Selektoren mit Klassen-Selektoren verschärfen die Selektionsbedingungen. Beispiel: `<h1 class="kursiv"> … </h1><p class="kursiv"> … </p>` hier selektiert CSS mit `h1.kursiv { … }` ausschliesslich die Heading. Der Absatz bleibt unverändert, obwohl er zur gleichen Klasse gehört.
